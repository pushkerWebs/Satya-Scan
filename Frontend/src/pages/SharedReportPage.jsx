  ...
  return (
    <div className="min-h-screen bg-[#FBE8CE] text-[#232B1B] font-sans pb-16 overflow-x-hidden">
      ...
      <div className="max-w-3xl mx-auto px-3 sm:px-6 py-5 sm:py-8 space-y-4 sm:space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
              <div className="flex-1 min-w-0 w-full">
                <span className="text-[10px] font-bold uppercase tracking-widest text-[#5C6650] block mb-2">
                  {isImage ? 'Image Authenticity Verdict' : 'Fact-Check Verification Verdict'}
                </span>
                ...
                {/* Summary */}
                {(check.finalAssessment || check.imageSummary || check.aiReasoning) && (
                  <p className="text-xs sm:text-sm text-[#232B1B] leading-relaxed font-medium mt-3 bg-[#FBE8CE] p-3 sm:p-4 rounded-xl border border-[#C3CC9B] break-words">
                    {check.finalAssessment || check.imageSummary || (Array.isArray(check.aiReasoning) ? check.aiReasoning.join(' ') : check.aiReasoning)}
                  </p>
                )}
              </div>
              ...
              <div className="w-full sm:w-auto flex flex-row sm:flex-col items-center justify-between sm:justify-center p-3.5 sm:p-4 rounded-xl bg-[#FBE8CE] border border-[#C3CC9B] shrink-0 sm:min-w-[110px]">
                ...
                <div className="text-right sm:text-center">
                  <span className="text-[10px] font-bold text-[#5C6650] uppercase tracking-wider block sm:mt-1">
                    {isImage ? 'Confidence' : 'Trust Score'}
                  </span>
        ...
        {check.originalText && (
          <div className="rounded-2xl p-4 sm:p-5 bg-[#E4DFB5] border border-[#C3CC9B]">
            <h3 className="text-xs font-bold text-[#5C6650] uppercase tracking-wider mb-2">
              {isImage ? 'Original Filename' : 'Original Claim Text'}
            </h3>
        ...
        {/* ── Image Forensics Breakdown (if image) ── */}
        {isImage && (
          <div className="rounded-2xl p-4 sm:p-5 bg-[#E4DFB5] border border-[#C3CC9B] space-y-4">
            <h3 className="text-xs font-bold text-[#232B1B] uppercase tracking-wider">
              Forensic Probabilities
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-3">
              {[
                { label: 'AI Generated', val: check.aiProbability ?? 0 },
                { label: 'Deepfake', val: check.deepfakeProbability ?? 0 },
                { label: 'Manipulated', val: check.manipulationProbability ?? 0 },
              ].map(({ label, val }) => (
                <div key={label} className="p-3 rounded-xl bg-[#FBE8CE] border border-[#C3CC9B] text-center">
                  <p className="text-lg sm:text-xl font-black" style={{ color: getTrustColor(100 - val) }}>{val}%</p>
                  <p className="text-[10px] font-bold text-[#5C6650] mt-0.5">{label}</p>
                </div>
              ))}
            </div>
            ...
            {check.findings?.length > 0 && (
              <div className="mt-3 space-y-2">
                <p className="text-[11px] font-bold text-[#5C6650] uppercase tracking-wider">Visual Observations:</p>
                <ul className="space-y-1.5">
                  {check.findings.map((f, i) => (
                    <li key={i} className="text-xs text-[#232B1B] flex items-start gap-2">
                      <span className="text-[#768E56] shrink-0">•</span>
